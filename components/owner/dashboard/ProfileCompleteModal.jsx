...
export default function ProfileCompleteModal({ open, onClose, items = [], onAction }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-[#e8e0cf] bg-[#ffffff] p-5 shadow-xl">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="text-base font-extrabold text-[#2b2118]">Lengkapi Profil Bisnis</h3>
            <p className="mt-0.5 text-xs text-[#857768]">
              {missing.length ? `${missing.length} bagian lagi menuju 100%` : "Semua lengkap — profil 100% 🎉"}
            </p>
          </div>
          <button onClick={onClose} aria-label="Tutup popup" className="flex h-8 w-8 items-center justify-center rounded-full border border-[#e0d5bd] text-[#6f6252] hover:text-[#3d2c1e]">
            <X size={15} />
          </button>
        </div>
        <ul className="mt-3 space-y-2">
          {items.map((it) => (
            <li key={it.key} className="flex items-center justify-between gap-2 rounded-xl border border-[#efe9d9] px-3 py-2">
              <span className="flex min-w-0 items-center gap-2 text-xs font-bold text-[#3d2c1e]">
                {it.done ? <CheckCircle2 size={15} className="shrink-0 text-[#1f6b4a]" /> : <Circle size={15} className="shrink-0 text-[#b8a888]" />}
              ...
              {!it.done && (
                <button
                  ...
                  className="shrink-0 rounded-full bg-[#3d2c1e] px-3 py-1.5 text-[11px] font-bold text-white hover:bg-[#2e2015]"
                >
                  Lengkapi
                </button>
              )}
