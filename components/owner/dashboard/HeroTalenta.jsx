...
export default function HeroTalenta({ photo, cafeName }) {
  return (
    <div className="flex gap-0 overflow-hidden rounded-2xl border border-[#e8e0cf] bg-[#ece2cd] py-4 pr-0 pl-5 shadow-[0_2px_12px_rgba(43,33,24,0.10)] sm:pl-6">
      <div className="min-w-0 flex-1 pr-5">
        <h1 className="font-display text-[1.7rem] leading-[1.05] font-semibold tracking-tight sm:text-3xl">
        ...
        <p className="mt-1 max-w-xl text-[13px] leading-5 text-[#6f6252]">
          ...
          dan pelatihan industri terkemuka. Bersama kerja.inc, rekrut lebih percaya diri.
        </p>
        <form action="/find-baristas" method="GET" className="mt-3 flex items-center gap-2">
          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-full border border-[#e0d5bd] bg-[#ffffff] px-4 py-2">
            ...
            <input
              ...
              className="h-6 w-full bg-transparent text-[13px] text-[#2b2118] placeholder:text-[#b6a98f] focus:outline-none"
            />
          </div>
          <button
            type="submit"
            className="inline-flex min-h-[38px] shrink-0 items-center rounded-full bg-[#3d2c1e] px-5 text-[13px] font-bold text-white hover:bg-[#2e2015]"
          >
            Cari Talenta
          </button>
        </form>
        <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
          ...
          {QUICK_FILTERS.map((f) => (
            <Link
              key={f}
              href="/find-baristas"
              className="rounded-full border border-[#e0d5bd] bg-[#ffffff] px-3 py-1 text-[11px] font-bold text-[#6f6252] hover:border-[#3d2c1e] hover:text-[#3d2c1e]"
            >
              {f}
            </Link>
          ))}
      ...
      <div className="relative hidden w-60 shrink-0 self-stretch sm:block lg:w-72">
        {photo ? (
          <>
