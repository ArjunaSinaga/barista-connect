  ...
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <form onSubmit={(e) => { e.preventDefault(); call("join_org_manager", { p_code: joinCode }, "Gabung PT berhasil").then((d) => { if (d) setJoinCode(""); }); }} className="rounded-2xl border border-[#e8e0cf] bg-white p-4">
          <h3 className="flex items-center gap-2 text-sm font-extrabold text-espresso"><UserPlus size={15} /> Gabung jadi manager (punya kode)</h3>
          <div className="mt-3 flex gap-2">
            <input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())} placeholder="XXXX-XXXX" maxLength={9} className="min-w-0 flex-1 rounded-full border border-[#e0d5bd] bg-[#faf7ef] px-4 py-2 font-mono text-sm font-bold tracking-wider uppercase focus:border-coffee focus:outline-none" />
        ...
        {orgs.map((o) => (
          <li key={o.id} className="rounded-2xl border border-[#e8e0cf] bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-extrabold text-espresso">{o.name} {o.needs_owner && <span className="ml-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700">tunggu owner</span>} {!o.isOwner && <span className="ml-1 rounded-full bg-[#e3f0e8] px-2 py-0.5 text-[10px] font-bold text-matcha">manager</span>}</p>
              {o.isOwner && (
                <button disabled={busy} onClick={async () => { const d = await call("regenerate_org_codes", { p_org: o.id }, "Kode baru aktif"); if (d) router.refresh(); }} className="inline-flex items-center gap-1 text-[11px] font-bold text-link hover:underline disabled:opacity-50">
                  <RefreshCw size={11} /> Acak kode
                </button>
              )}
            </div>
            {o.isOwner && (
              <div className="mt-2 space-y-1 text-[11px] text-espresso-soft">
                <p className="flex items-center gap-2">Kode owner: <span className="rounded bg-[#efe9d9] px-1.5 py-0.5 font-mono font-bold text-espresso">{o.owner_code}</span>
                  <button onClick={() => copy(o.owner_code)} className="inline-flex items-center gap-1 font-bold text-link hover:underline"><Copy size={11} /> Salin</button></p>
                <p className="flex items-center gap-2">Kode manager: <span className="rounded bg-[#efe9d9] px-1.5 py-0.5 font-mono font-bold text-espresso">{o.manager_code}</span>
                  <button onClick={() => copy(o.manager_code)} className="inline-flex items-center gap-1 font-bold text-link hover:underline"><Copy size={11} /> Salin</button></p>
              </div>
            )}
            <div className="mt-3">
              <p className="flex items-center gap-1.5 text-xs font-extrabold text-espresso"><UserPlus size={13} /> Manager ({(o.members ?? []).length})</p>
              {(o.members ?? []).length === 0 && <p className="mt-1 text-[11px] text-espresso-soft">Belum ada manager. Share kode manager.</p>}
