...
function clean(t){
  let s=(t??"").replace(/<\|[^|]*\|>/g,"");
  // buang blok thinking Qwen walau tag tidak ditutup
...
async function callOpenAICompatible(p,sys,usr){
  const body={model:p.model,messages:[{role:"system",content:sys},{role:"user",content:usr}],[p.field]:220,temperature:0.4};
  // matikan chain-of-thought di model reasoning (Qwen dkk) agar tidak bocor ke output
  if(/qwen|qwq|deepseek-r1|reasoning/i.test(p.model)){body.reasoning_effort="none";body.reasoning_format="hidden";body.enable_thinking=false;}
