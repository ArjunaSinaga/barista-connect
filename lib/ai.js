...
function clean(t){
  let s=(t??"").replace(/<think>[\s\S]*?<\/think>/gi,"").replace(/<\|[^|]*\|>/g,"").trim();
  // buang echo prompt / reasoning Qwen yang bocor
  const lines=s.split("\n");
  const bad=/^\s*([-*#>]*\s*)?(\*\*?(role|facts|riwayat|aturan|transcript|facts?|system)\b|here'?s .*thinking|thinking process|analyze user input|langkah\b)/i;
  while(lines.length && bad.test(lines[0])) lines.shift();
  s=lines.join("\n").trim();
  // jika masih ada sisa header prompt di tengah, ambil bagian setelah baris kosong terakhir yang terlihat seperti draf
  const cut=s.search(/\n\s*\n(?=[A-ZÀ-Ža-z].{5,120}$)/);
  return s.trim();
}
const PROVIDERS=[
...
async function callOpenAICompatible(p,sys,usr){
 const res=await fetch(p.url,{method:"POST",headers:{Authorization:`${p.header} ${process.env[p.key]}`,"Content-Type":"application/json"},body:JSON.stringify({model:p.model,messages:[{role:"system",content:sys},{role:"user",content:usr}],[p.field]:220,temperature:0.4})});
