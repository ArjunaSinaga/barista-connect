import { z } from "zod";
import { EMPLOYMENT_TYPES as EMP_TYPES, EMPLOYMENT_LABELS as EMP_LABELS } from "./constants";

// Satu sumber: constants.js (dulu duplikat + label "Panggilan" beda dgn "Casual").
export const EMPLOYMENT_TYPES = EMP_TYPES.map((t) => t.value);
export const EMPLOYMENT_LABELS = EMP_LABELS;
...
export const signUpSchema = z.object({
  ...
  phone: waSchema.optional().or(z.literal("")),
  password: z.string().min(8, "Password minimal 8 karakter").max(72).regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/, "Harus ada huruf besar, kecil & angka"),
...
// Profil owner ringkas (detail alamat/peta/WA pindah ke tiap cafe)
...
// Form Data Bisnis (profil owner): tanpa peta — alamat & koordinat diatur per cafe.
// WA opsional: tanpa isi pun profil tetap bisa 100%.
export const ownerBusinessSchema = z.object({
  business_name: z.string().min(2, "Nama bisnis minimal 2 karakter").max(120),
  location: z.string().min(2, "Kota / lokasi teks wajib diisi"),
  whatsapp: waSchema.optional().or(z.literal("")),
});
...
export const jobPostSchema = z
  .object({
    cafe_id: z.string().uuid("Pilih cafe dulu"),
...
export const profileUpdateSchema = z.object({
  ...
  location_place: z.string().min(2, "Lokasi wajib diisi"),
  years_of_experience: z.coerce.number().int().min(0).max(50).optional().default(0),
  exp_years: z.coerce.number().int().min(0).max(50).optional().default(0),
  exp_months: z.coerce.number().int().min(0).max(11).optional().default(0),
  whatsapp: waSchema.optional().or(z.literal("")),
  ...skillsShape,
